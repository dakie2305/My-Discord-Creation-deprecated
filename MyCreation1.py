...
async def on_message(message):
    ...
    if functions.contains_substring(message.content.lower(), bots_creation1_name):
        ...
        if message.author.id == user_darkie['user_id']:
            roleplay_ele = f"{initial_instruction} **Ngươi tên là Creation 1, và ngươi đang nói chuyện với bề trên Đấng Tạo Hoá Darkie. Hãy thật kính trọng.**\n"
        response = model.generate_content(f"{roleplay_ele} Hãy trả lời nội dung sau với tính cách trên: {formated_string}")
        ...
        print(f"Username {message.author.name} directly call Creation 1")
